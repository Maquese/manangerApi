...
namespace ManangerAPI.Data.Repositorios
{
    public class GestorRepositorio : Repositorio<Gestor>, IGestorRepositorio
    {
        ...
        {
            return _contexto.Gestor.Where(x => (x.Aprovado == null || x.Aprovado) && x.Status == 0).ToList();
        }

        public IList<Gestor> ListarPorAnalise(bool analisado)
        {
            return _contexto.Gestor.Where(x => x.Analisado == analisado && x.Status == 0).ToList();
        }

        public IList<Gestor> ListarPorAprovacao(bool aprovado)
        {
             return _contexto.Gestor.Where(x => x.Aprovado == aprovado && x.Status == 0).ToList();
        }
    }
}