...
namespace ManangerAPI.Data.Repositorios
{
    public class PrestadorDeServicoRepositorio : Repositorio<PrestadorDeServico>, IPrestadorDeServicoRepositorio
    {
        ...
        {
            return _contexto.PrestadorDeServico.Where(x => (x.Aprovado == null || x.Aprovado) && x.Status == 0).ToList();
        }

        public IList<PrestadorDeServico> ListarPorAnalise(bool analisado)
        {
            return _contexto.PrestadorDeServico.Where(x => x.Analisado == analisado && x.Status == 0).ToList();
        }

        public IList<PrestadorDeServico> ListarPorAprovacao(bool aprovado)
        {
           return _contexto.PrestadorDeServico.Where(x => x.Aprovado == aprovado && x.Status == 0).ToList();
        }
    }
}